...
bool my_robot_interfaces__srv__set_led__request__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // turn_on_off
    PyObject * field = PyObject_GetAttrString(_pymsg, "turn_on_off");
    if (!field) {
      return false;
    }
    assert(PyBool_Check(field));
    ros_message->turn_on_off = (Py_True == field);
    Py_DECREF(field);
  }
...
PyObject * my_robot_interfaces__srv__set_led__request__convert_to_py(void * raw_ros_message)
{
  ...
  {  // turn_on_off
    PyObject * field = NULL;
    field = PyBool_FromLong(ros_message->turn_on_off ? 1 : 0);
    {
      int rc = PyObject_SetAttrString(_pymessage, "turn_on_off", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
